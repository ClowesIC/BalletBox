...
List::List(List& orig, int lowIndex, int highIndex) {
    myCapacity = orig.myCapacity; //set new list capacity equal to original list
    mySize = highIndex - lowIndex +1;
    myArrayPtr = new ElementType[myCapacity];

    //copy elements into new array that fit into the slected user min and max indexes
    for (int i = lowIndex, j = 0; i < highIndex+1; i++, j++) {
...
void List::Append(List& ListToAppend) {
    myCapacity = myCapacity + ListToAppend.myCapacity; //expand capacity
   
    ElementType *newArrayPtr = new ElementType[myCapacity];
    ...
    mySize = mySize + ListToAppend.mySize;
    delete[] myArrayPtr;
