...
// Name: Ian Clowes
// Section: CMPSC 122
// Assignment: InLab07 - linked list
//
...
int GetLength(Node* first) {
	int NodeCounter = 0;
	while (first != nullptr) { //will check through each node of first linked list until reaching nullprt (end)
		NodeCounter++;
		first = first->next;
	}
...
bool GetElement(Node* first, int position, Type& data) {
	for (int i = 0; i <= position; i++) { //iterate to position
...
void InsertToSortedLinkedList(Node*& first, Type data) {
	...
	if (first == nullptr || first->data >= data) {
		newNode->next = first;
		first = newNode;
		return;
	}
...
int main() {
	...
	}
	{
		...
		display(first);
		cout << endl;
		cout << "Try to insert 100 into this linked list, the list becomes:" <<
			endl;
